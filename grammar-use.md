...
-   For the [WebSocket interface](/docs/services/speech-to-text-icp?topic=speech-to-text-icp-websockets), you first specify the customization ID with the `language_customization_id` parameter of the `/v1/recognize` method. You use this method to establish a WebSocket connection with the service.
    ...
    ```
    {: codeblock}
-   For the [synchronous HTTP interface](/docs/services/speech-to-text-icp?topic=speech-to-text-icp-http), pass both parameters with the `POST /v1/recognize` method.
    ...
    ```
    {: pre}
-   For the [asynchronous HTTP interface](/docs/services/speech-to-text-icp?topic=speech-to-text-icp-async), pass both parameters with the `POST /v1/recognitions` method.
