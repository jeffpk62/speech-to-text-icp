...
When you make a speech recognition request, you must use the `Content-Type` parameter to specify the format, or MIME type, of the audio that you pass to the service. The following sections introduce basic audio terminology. They describe the audio formats that are supported by {{site.data.keyword.ibmwatson}} {{site.data.keyword.speechtotextshort}}: Customer Care. And they discuss the use of compression to maximize the amount of audio that you can pass to the service.
